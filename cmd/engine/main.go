...
func main() {
	...
	// 启动告警处理
	go processAlerts(alertChan, db, natsClient, llmClient)
	...
	// 在main函数中添加爬虫数据订阅
	// 订阅爬虫数据
	_, err = natsClient.SubscribeCrawlerData(func(quote model.StockQuote) {
	// 评估规则
	engine.Evaluate(quote)
	})
	if err != nil {
	log.Printf("订阅爬虫数据失败: %v\n", err)
	}
	// 等待中断信号
	sigChan := make(chan os.Signal, 1)
	signal.Notify(sigChan, syscall.SIGINT, syscall.SIGTERM)
...
// 处理告警事件
func processAlerts(alertChan <-chan model.AlertEvent, db *database.TimescaleDB, natsClient *messaging.NATSClient, llmClient *llm.LLMClient) {
	for alert := range alertChan {
		...
		// 发布异动事件到NATS
		if err := natsClient.PublishAlert(alert, llmClient); err != nil {
